...
- **tauri-plugin-global-shortcut 2** — 전역 단축키 (`⌘⇧R` Reference 저장, 사용자 변경 가능) ← 추가됨
...
│   │   │   └── settings.rs         # get_settings, update_settings, get_classification_rules
│   │   │                           # add_classification_rule, delete_classification_rule
│   │   ├── state/
│   │   │   ├── mod.rs
...
│   │   │   ├── shortcut.rs         # 전역 단축키 등록/변경 (tauri-plugin-global-shortcut)
...
│   │   │   └── settings.rs         # AppSettings (retention_days, shortcut_save_ref)
│   │   │                           # ClassificationRule (추가/삭제 지원)
...
│   │   └── V2__archive.sql
│   ├── Cargo.toml
│   └── tauri.conf.json   # 창 목록: dropdown, dashboard, save-reference, settings
│
├── src/
│   ├── components/
│   │   ├── Dropdown/
...
│   │   │   └── SavedReferences.tsx
│   │   └── Settings/
│   │       ├── ShortcutSettings.tsx    # 단축키 변경 UI
│   │       ├── RetentionSettings.tsx   # 보관 기간 선택 UI
│   │       ├── RulesSettings.tsx       # 분류 규칙 CRUD UI
│   │       └── AutoLaunchSettings.tsx  # 자동 실행 토글 UI
│   ├── pages/
│   │   ├── Dropdown.tsx
│   │   ├── Dashboard.tsx
│   │   ├── SaveReference.tsx           # 별도 팝업 창 페이지
│   │   └── Settings.tsx                # 설정 창 페이지
│   ├── hooks/
│   │   ├── useSession.ts
│   │   ├── useActivity.ts
│   │   └── useMetrics.ts
