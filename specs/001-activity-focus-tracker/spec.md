...
### User Story 6 - 설정 관리 (Priority: P6)

사용자가 설정 창에서 전역 단축키, 데이터 보관 기간, 분류 규칙, 자동 실행 여부를 설정한다.
트레이 우클릭 메뉴의 "설정 열기" 또는 전역 단축키(`⌘⇧R`)로 Reference 저장 팝업을 직접 열 수 있다.

**Why this priority**: 핵심 기능이 완성된 후 사용자 커스터마이징을 제공하는 확장 기능이다. P1~P5 완료 후 추가된다.

**Independent Test**: 설정 창에서 분류 규칙을 추가하면 이후 활동 분류에 즉시 반영되는지 확인한다. 전역 단축키를 변경하면 새 단축키로 동작하는지 확인한다.

**Acceptance Scenarios**:

1. **Given** 트레이 우클릭 메뉴가 표시된 상태, **When** "설정 열기"를 클릭하면, **Then** 설정 창이 열린다.
2. **Given** 설정 창이 열린 상태, **When** Reference 저장 단축키를 변경하고 저장하면, **Then** 새 단축키로 Reference 저장 팝업이 열린다.
3. **Given** 설정 창이 열린 상태, **When** 데이터 보관 기간을 변경하면, **Then** 이후 아카이빙 실행 시 변경된 기간이 적용된다.
4. **Given** 설정 창의 분류 규칙 목록, **When** 새 규칙(도메인 + 분류)을 추가하면, **Then** 이후 해당 도메인 활동이 지정된 분류로 기록된다.
5. **Given** 설정 창의 분류 규칙 목록, **When** 기존 규칙을 삭제하면, **Then** 이후 해당 도메인은 Neutral로 분류된다.
6. **Given** 브라우저가 활성화된 상태, **When** `⌘⇧R` 단축키를 누르면, **Then** Reference 저장 팝업이 현재 URL이 채워진 상태로 열린다.
7. **Given** 설정 창에서 "로그인 시 자동 실행"을 활성화한 상태, **When** 시스템을 재시작하면, **Then** 앱이 자동으로 실행된다.
...
- **FR-015**: 메뉴바 **좌클릭** 시 드롭다운(NSPanel)이 열려야 하며, 세션 타이머, Focus % 헤더, 세션 시작/종료 버튼, 도넛 차트(Focus/Neutral/Distraction 비율), 현재 앱 이름, 최근 활동 리스트(상위 5개, 앱 이름 + 퍼센트), Dashboard 열기 버튼이 포함되어야 한다. 드롭다운은 어두운 테마(#1c1c1e 배경, blur 효과)를 사용한다. 드롭다운 외부 영역 클릭 시 자동으로 닫혀야 한다.
- **FR-015a**: 메뉴바 **우클릭** 시 컨텍스트 메뉴가 표시되어야 한다: "설정 열기", "focaro 종료" 항목 포함.
- **FR-016**: 사용자는 브라우저 활동 중 현재 URL을 제목과 선택적 태그와 함께 Reference로 저장할 수 있어야 한다. 저장 UI는 드롭다운 내 인라인이 아닌 **별도 팝업 창**(save-reference 윈도우)으로 표시되어야 한다. 팝업은 현재 URL을 자동으로 채운 상태로 열려야 한다.
...
- **FR-019**: 앱은 백그라운드에서 지속 실행되는 메뉴바 앱으로 동작해야 한다. 드롭다운 창은 NSPanel(tauri-nspanel)로 변환되어야 하며, 전체화면 앱 위에서도 표시되어야 한다(NSWindowCollectionBehaviorFullScreenAuxiliary). 드롭다운 열림 시 현재 활성 앱의 포커스를 빼앗지 않아야 한다(NSWindowStyleMaskNonActivatingPanel).
...
- **FR-023**: 사용자는 로우 Activity 데이터 보관 기간을 설정할 수 있어야 한다 (기본값: 30일). 설정은 설정 창(FR-026)에서 변경한다.
- **FR-026**: 시스템은 전역 단축키(글로벌 단축키)로 Reference 저장 팝업을 열 수 있어야 한다. 기본 단축키는 `⌘⇧R`이며, 사용자가 설정 창에서 변경할 수 있어야 한다. 단축키는 앱이 포커스되지 않은 상태에서도 동작해야 한다.
- **FR-027**: 시스템은 설정 창을 제공해야 한다. 설정 창은 트레이 우클릭 메뉴의 "설정 열기"로 접근한다. 설정 창에 포함될 항목:
  - **단축키**: Reference 저장 단축키 (기본 `⌘⇧R`), 드롭다운 토글 단축키 (선택, 기본값 없음)
  - **데이터 보관 기간**: 7일 / 30일 / 90일 / 무제한 선택 (기본 30일)
  - **분류 규칙 관리**: 도메인별 Focus/Neutral/Distraction 규칙 추가·수정·삭제
  - **로그인 시 자동 실행**: 시스템 로그인 시 자동으로 앱 실행 여부 (기본 비활성)
- **FR-028**: 분류 규칙 변경은 즉시 이후 활동 분류에 반영되어야 한다. 기존 저장된 활동의 재분류는 MVP 범위 밖이다.
...
- Q: 드롭다운 최근 활동 리스트 표시 개수는? → A: 상위 5개 (앱별 누적 시간 기준 내림차순).

### Session 2026-03-20 (UI 리디자인)

- Q: 드롭다운 테마는? → A: 어두운 테마 (#1c1c1e 배경, blur 효과). 기존 라이트 테마에서 변경.
- Q: 드롭다운에 보여줄 활동 정보는? → A: 현재 앱 이름 + 도넛 차트(Focus %) + 앱별 누적 시간 리스트(상위 5개).
- Q: 메뉴바 우클릭 시 동작은? → A: "focaro 종료" 버튼이 있는 컨텍스트 메뉴를 표시한다.
- Q: 드롭다운이 전체화면 앱 위에 표시되어야 하는가? → A: 예. NSPanel(tauri-nspanel)을 사용하여 모든 스페이스/전체화면 위에 표시되어야 한다.

### Session 2026-03-21 (설정 및 팝업 추가)

- Q: Reference 저장 UI 방식은? → A: 드롭다운 내 인라인이 아닌 별도 팝업 창(save-reference 윈도우)으로 변경. 버튼 클릭 또는 전역 단축키로 팝업 열기.
- Q: 전역 단축키 기본값은? → A: `⌘⇧R` (사용자 변경 가능)
- Q: 설정 창 접근 방식은? → A: 트레이 우클릭 메뉴 "설정 열기" 항목으로 접근
- Q: 설정 항목은? → A: 단축키 설정, 데이터 보관 기간(7/30/90일/무제한), 분류 규칙 CRUD, 로그인 시 자동 실행 토글
